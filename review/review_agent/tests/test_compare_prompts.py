...
def test_obligation_prompt_quote_downgrade_warning():
    text = (_PROMPTS / "obligation_compare.md").read_text(encoding="utf-8")
    assert "downgrades to INCONCLUSIVE" in text
    assert "preferred_position" in text
    assert "adopts or references" in text


def test_obligation_prompt_batch_coverage():
    text = (_PROMPTS / "obligation_compare.md").read_text(encoding="utf-8")
